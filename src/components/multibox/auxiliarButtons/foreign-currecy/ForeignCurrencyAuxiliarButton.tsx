'use client'
import styles from './ForeignCurrencyAuxiliarButton.module.css';
...
export default function ForeignCurrencyAuxiliarButton(props: { currencyName: string }) {
    ...
    const onClickProvider = handleSubmit(async (data) => {
        try {
            ...
            if (response.ok) {
                ...
                router.replace(`/multibox/foreign-currency/${props.currencyName}`)
            } else {
    ...
    function converFormData(data: ManualTransactionForm): ManualTransactionRequest {
        return {
            currencyBoxName: props.currencyName,
    ...
    return (
        <form onSubmit={onClickProvider} className={styles.formBase}>
            ...
            <div>
                <button onClick={()=>router.replace(`/multibox/foreign-currency/${props.currencyName}`)}>Cancelar</button>
